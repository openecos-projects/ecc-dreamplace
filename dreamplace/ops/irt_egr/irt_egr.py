##
# @file   nctugr_binary.py
# @author Yibo Lin
# @date   Jan 2020
#

import os
import stat
import sys
import logging
import torch
import pdb
import numpy as np

logger = logging.getLogger(__name__)


class IRT_eGR(object):
    def __init__(self, params, placedb):
        self.params = params
        self.placedb = placedb

    def __call__(self, pos, stage, resolve_congestion):
        return self.forward(pos, stage, resolve_congestion)

    def forward(self, pos, stage, resolve_congestion):
        pos_cpu = pos.detach().cpu().numpy()

        num_movable_nodes = self.placedb.num_movable_nodes
        node_x = pos_cpu[:num_movable_nodes].copy()
        node_y = pos_cpu[
            self.placedb.num_nodes : self.placedb.num_nodes
            + num_movable_nodes
        ].copy()
        cell_padding_x = getattr(self.params, "cell_padding_x", 0)
        if cell_padding_x >= 0:
            node_x += cell_padding_x

        # unscale locations
        unscale_factor = 1.0 / self.params.scale_factor
        node_x = (
            node_x[:num_movable_nodes] * unscale_factor
            + self.params.shift_factor[0]
        )
        node_y = (
            node_y[:num_movable_nodes] * unscale_factor
            + self.params.shift_factor[1]
        )

        # update raw database
        self.placedb.write_placement_back(node_x, node_y)
        utilization_map_py = self.placedb.pydb.getCongestionMap(
            "max", stage, resolve_congestion)
        utilization_map_np = np.array(utilization_map_py, dtype=np.float32)
        utilization_map = torch.from_numpy(utilization_map_np).to(pos.device).T
        utilization_map = utilization_map.contiguous()

        return utilization_map
